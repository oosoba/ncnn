function err_rates = CNEMSketchLogged(trials_sz, log_file)
% Path of toolbox directory
% trials_sz =  1e4:1e4:6e4;
toolbox_dir='./';
...
opts.noise_var = 1;	% Noise variance
opts.noise_pdf = 'uniform';	% One of 'gaussian' or 'uniform'
opts.anneal_fact = 5;	% Noise annealing factor (positive real number, preferably 1 or 2)
opts.maxMC = 1;
...
for j = 1:length(trials_sz)
    ...
    % Train the CNN
    cnn = cnntrain_nem(cnn, trainX, trainY, opts);
    % Test the CNN
